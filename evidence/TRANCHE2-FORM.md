...
Green CI runs (lint, format, typecheck, 215 Vitest tests incl. the stdio MCP suite and the skill walkthrough, MCP schema drift check, skill validator, tsc build, offline demo, committed reports and artifacts regenerated and diffed; docs-site build; Rust fmt/clippy/tests and three wasm builds asserted against the deployed hashes): https://github.com/kunal-drall/policywright/actions/runs/33654005983 (the close-out commit, 215 tests) and https://github.com/kunal-drall/policywright/actions/runs/33647377983 and https://github.com/kunal-drall/policywright/actions/runs/33631174115 and https://github.com/kunal-drall/policywright/actions/runs/33622880981 and https://github.com/kunal-drall/policywright/actions/runs/33618871930 and https://github.com/kunal-drall/policywright/actions/runs/33561221471

Docs site: https://policywright.lemmalabs.space ; its Tranche 2 pages as committed — the roadmap with per-deliverable status https://github.com/kunal-drall/policywright/blob/main/site/src/content/docs/roadmap.mdx and the reference pages for the MCP tools, the skill, the smart-account install and the dry-run harness https://github.com/kunal-drall/policywright/tree/main/site/src/content/docs/reference
...
Recorded but not blocking: the public docs site is at its 2026-08-06
deployment — the Vercel project has no git integration, so pushes do not
publish, and the Vercel CLI on this machine holds no credentials (FACTS
§17.4). The form therefore cites the site's Tranche 2 pages by their
committed sources on GitHub, which is verifiable today. To cite the live
pages instead, redeploy first (`cd site && vercel login && vercel --prod`;
the directory is already linked to the `policywright-docs` project) and
check `https://policywright.lemmalabs.space/roadmap/` shows the Tranche 2
table. Also: GitHub Issues remain disabled on the repository
