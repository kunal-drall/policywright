...
**CI run for this work:**
[run 33654005983](https://github.com/kunal-drall/policywright/actions/runs/33654005983)
— all three jobs green on commit `e3f0bfe` (`build`: lint → format →
typecheck → 215 tests incl. the stdio MCP suite and the skill walkthrough →
MCP schema drift check → `skills-ref validate` → `tsc` build → demo → live
reports, side-by-side artefacts and the demo-script artifact diffed against
the committed copies; `site`: docs build with the three new pages;
`contracts`: fmt → clippy → Rust tests → three wasm builds asserted against
the deployed hashes). Dispatched manually (fork). A fresh clone of the same
commit was also run through the node and site gates locally the same day.
