...
<Mermaid
  ...
  emitter --> contextrule["context-rule.json"]
...
- **`FrequencyLimitPolicy.rs`** — the custom policy in Rust, implementing OZ's real
  ...
  byte-identical to the compiled-and-tested crate at `contracts/frequency-limit-policy`
  (equality locked by `test/rust-policy.test.ts`). Every generated file is
