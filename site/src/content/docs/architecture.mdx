---
title: Architecture
description: The v0 pipeline in depth — recording, synthesis, emission, and simulation.
---
...
<Mermaid
  code={`flowchart LR
  txhash(["tx hash"]) --> rpc["sources/rpc"]
  ...
  emitter --> rust["FrequencyLimitPolicy.rs"]
...
- **`recorded.ts`** re-loads a previously saved recording (`synth --input <file>`).
  ...
  every shape verified against the committed captures,
...
- **Scope** — the distinct `(contract, fn)` pairs observed, in first-seen order, plus a
  ...
  per byte;
  [FACTS.md §2.3](https://github.com/kunal-drall/policywright/blob/main/docs/FACTS.md)).
...
- **`FrequencyLimitPolicy.rs`** — the custom policy in Rust, implementing OZ's real
  ...
  **ILLUSTRATIVE / UNAUDITED — NOT DEPLOY-READY**; the summary carries the same note.
...
simulates in enforcing mode, signs through a labelled signing surface (local `.env` key
fallback today; a wallet signs the same transaction), and submits; `verify.ts` reads the
account's rules and policy params back through simulated getters and diffs them. Details:
...
`buildScenarios` builds the unobserved-route case from the REAL observed swap re-routed
through a **probe token** — the recording network's native XLM Stellar Asset Contract by
default (`--probe-token` overrides) — and `renderReport` prefixes every report with its
provenance: recording, generated policy set, mode, decision legend, token addresses.
...
**Stage 3b is not reachable from here**: there is no install or deploy tool, the server never
signs, and it needs no secret — the artifact an agent produces is what the human installs with
the CLI. The [skill](/reference/skill/) is a conversation over these four tools.
