...
| `cd site && npm run build` | exit 0 — **20 pages** (16 before + 4 use cases), Pagefind "Found 40 HTML files", `sitemap-index.xml`; the only warning is Vite's chunk-size note for the Mermaid bundle                                                                                                                                                                                                                                                                                                                                                                                              | local build (×5 in the session; the first failure was four pages written to a nested path by a drifted shell cwd, fixed) |
...
| Status badges and banner   | `StatusBadge` gained an optional `note` ("Shipped (testnet)", "Planned (walkthrough — Tranche 3, target 30 Nov 2026)"); every one of the 20 pages renders a badge and the banner sentence verbatim (checked in the built HTML); no page says "In development"                                                                                                                                                                                                                                                                                                        | Python over `dist/**/index.html`                                                                                         |
